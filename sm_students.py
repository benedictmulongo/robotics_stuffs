...
class StateMachine(object):
    ...
    def __init__(self):
        ...
        self.pick_srv_n = rospy.get_param(rospy.get_name() + '/pick_srv')
        self.place_srv_n = rospy.get_param(rospy.get_name() + '/place_srv')
        ...
        self.aruco_pose_topic_n = rospy.get_param(rospy.get_name() + '/aruco_pose_topic')
        self.cube_pose_n = rospy.get_param(rospy.get_name() + '/cube_pose')
        
        # Subscribe to topics
        
        # Wait for service providers
        rospy.wait_for_service(self.mv_head_srv_nm, timeout=30)
        rospy.wait_for_service(self.pick_srv_n, timeout=30)
        rospy.wait_for_service(self.place_srv_n, timeout=30)

        # Instantiate publishers
        self.cmd_vel_pub = rospy.Publisher(self.cmd_vel_top, Twist, queue_size=10)

        self.cube_pose_pub = rospy.Publisher(self.aruco_pose_topic_n, PoseStamped, queue_size=10)
    ...
    def turn_into_PoseStamped(self, string_pos):
        self.pick_pose = string_pos.split(',')
    ...
    def check_states(self):
        ...
        while not rospy.is_shutdown() and self.state != 4:
            ...
            if self.state == 1:
                # Publish the goal
                #pose_msg = rospy.wait_for_message(self.pkpose_top, PoseStamped, 5)
                #self.cube_pose_pub.publish(pose_msg)
                
                pose_msg = self.turn_into_PoseStamped(self.cube_pose_n)
                self.cube_pose_pub.publish(pose_msg)
                
                try:
                    rospy.loginfo("%s: Picking the cube...", self.node_name)
                    pick_up_srv = rospy.ServiceProxy(self.pick_srv_n, SetBool)
                    pick_up_req = pick_up_srv()
                    if pick_up_req.success == True:
                        rospy.loginfo("%s: Proceeding to state 2...", self.node_name)
                        self.state = 2
                    else:
                        rospy.loginfo("%s: Did not complete pick...", self.node_name)
                        self.state = 5
                    ...
                    print 'Service call to pick_up server failed: ', e
            ...
            if self.state == 3:
                try:
                    ...
                    place_srv = rospy.ServiceProxy(self.place_srv_n, SetBool)
                    place_req = place_srv()
                    if place_req.success == True:
                        rospy.loginfo("%s: FInnished...", self.node_name)
                        self.state = 6
                    else:
                        rospy.loginfo("%s: Did not complete place...", self.node_name)
                        self.state = 5
            ...
            # State 6:  Move the robot "manually" to chair
            if self.state == 7:
                move_msg = Twist()
                move_msg.angular.z = -1
